  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 font-sans">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-white p-6 rounded-[2rem] border border-border shadow-sm">
          <div className="text-muted-foreground text-sm font-bold mb-1 flex items-center gap-2"><Zap className="w-4 h-4" /> Revenue</div>
                ...
                <tbody className="text-sm font-medium">
                  {stores.map(store => (
                    <tr key={store.id} className="border-b border-border/50">
                      ...
                      <td className="py-4">
                         <span className={`px-2 py-1 rounded text-xs font-bold ${store.status === 'Verified' ? 'bg-primary/10 text-primary' : 'bg-amber-100 text-amber-700'}`}>
                           {store.status}
                         </span>
                      </td>
                      <td className="py-4 text-right">
                         {store.status === 'Pending Verification' ? (
                           <button onClick={() => verifyStore(store.id)} className="text-primary hover:underline font-bold flex items-center justify-end gap-1 w-full">
