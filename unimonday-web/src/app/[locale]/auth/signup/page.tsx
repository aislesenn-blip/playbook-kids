...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#F8F6F3] flex flex-col items-center justify-center p-4">
      ...
      >
        <h1 className="text-2xl font-semibold tracking-tight text-zinc-900 mb-2">{t('signupTitle', { fallback: 'Parent Setup' })}</h1>
        <p className="text-zinc-500 mb-8 text-sm">{t('signupSubtitle', { fallback: 'Create an account to manage their learning.' })}</p>
        ...
        <form className="space-y-4" onSubmit={(e) => {
          ...
          <div>
            <input
              type="text"
              placeholder={t('parentName', { fallback: 'Parent Name' })}
          ...
          <div>
            <input
              type="password"
              placeholder={t('createPassword', { fallback: 'Create Password' })}
              className="w-full px-5 py-3 bg-zinc-50 border border-zinc-100 rounded-xl focus:ring-1 focus:ring-zinc-900 focus:border-zinc-900 outline-none transition-all placeholder:text-zinc-400 text-zinc-900"
            />
          </div>
          ...
          <motion.button whileTap={{ scale: 0.98 }} disabled={isSubmitting} className="w-full py-3 bg-zinc-900 text-white rounded-xl font-medium hover:bg-zinc-800 transition-colors mt-6 flex justify-center items-center h-[52px]">
            {isSubmitting ? <Loader2 className="w-5 h-5 animate-spin" /> : t('continueDemo', { fallback: 'Continue to Interactive Demo' })}
          </motion.button>
        </form>

        <p className="text-center text-zinc-500 mt-6 text-sm">
          {t('alreadyHaveAccount', { fallback: 'Already have an account?' })} <Link href={`/${locale}/auth/login`} className="text-zinc-900 font-medium hover:underline">{t('loginButton')}</Link>
        </p>
      </motion.div>
    </div>
  );
