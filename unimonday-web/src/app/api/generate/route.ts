...
export async function POST(req: Request) {
  try {
    ...
    if (action === 'architect') {
       ...
       messages = [{ role: 'user', content: `You are an expert Apple/Gemini-level Software Architect.\n\n${BASE_BUILDER_RULES.replaceAll('${UNSPLASH_ACCESS_KEY}', UNSPLASH_ACCESS_KEY)}\n\nTASK: Modify the following application codebase based strictly on the user's edit request.\n\nUSER EDIT REQUEST:\n${prompt}\n\nCURRENT HTML:\n${html}\n\nCURRENT CSS:\n${css}\n\nCURRENT JS:\n${prd || ''}\n\nOUTPUT FORMAT MUST BE VALID JSON enclosed in markdown code blocks. Return ONLY the files that were modified. If a file does not need modifications to fulfill the request, DO NOT include it in the JSON output.\n{ "html": "[Optional fully updated HTML]...", "css": "[Optional fully updated CSS]...", "js": "[Optional fully updated JS]..." }` }];
    } else {
