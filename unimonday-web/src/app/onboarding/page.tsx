  ...
  const handleNext = () => {
    if (step < 4) setStep(step + 1);
      ...
      const newProfile: UserProfile = {
        ...
        name: form.name,
        role: form.role,
        targetLanguage: form.language as Language,
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-white flex flex-col">
      <div className="flex-1 max-w-2xl mx-auto w-full px-4 py-8 flex flex-col justify-center relative">
        ...
        <div className="w-full h-2 bg-gray-100 rounded-full mb-12 overflow-hidden">
          <div className="h-full bg-[#DDA359] transition-all duration-500" style={{ width: `${(step / 4) * 100}%` }} />
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 2 && (
            <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col gap-8">
               <div className="text-center">
                <h2 className="text-3xl font-black mb-4">What do you want to learn?</h2>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {['English', 'Spanish', 'French', 'Chinese', 'German', 'Swahili'].map(lang => (
                   <button
                    key={lang}
                    onClick={() => setForm({...form, language: lang as Language})}
                    className={`p-6 rounded-2xl border-2 text-center transition-all ${form.language === lang ? 'border-[#DDA359] bg-[#DDA359]/5 scale-105' : 'border-gray-200 hover:border-gray-300'}`}
                  >
          ...
          {step === 3 && (
           ...
           {step === 4 && (
            <motion.div key="step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col gap-8">
               <div className="text-center">
                <h2 className="text-3xl font-black mb-4">What is your name?</h2>
        ...
        <div className="mt-12 flex justify-end">
          <button
            onClick={handleNext}
            disabled={(step === 2 && !form.language) || (step === 3 && !form.level) || (step === 4 && !form.name.trim())}
          ...
          >
            {step === 4 ? 'Enter Universe' : 'Continue'} <ArrowRight className="w-5 h-5" />
          </button>
        </div>
      </div>
    </div>
