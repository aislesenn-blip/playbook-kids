    ...
    <nav className="fixed top-0 left-0 right-0 h-20 bg-[#F8F6F3]/95 backdrop-blur-md border-b border-zinc-100 z-[100] flex items-center px-4 md:px-8 justify-between shadow-[0_4px_40px_rgba(0,0,0,0.02)]">
      ...
      {profile ? (
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <Link href={`/${locale}/profile`} className="relative ml-2 md:ml-0 block">
        ...
        <div className="flex items-center gap-4">
           <LanguageSwitcher />
           <Link href={`/${locale}/auth/login`} className="font-medium text-zinc-600 hover:text-zinc-900 transition-colors hidden sm:block">
             {t('Hero.login')}
           </Link>
