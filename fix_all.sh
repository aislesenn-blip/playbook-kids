#!/bin/bash
# Re-apply chat patch since restore wiped it
...
# Replace specific chunks of old code with new code
cat << 'PY_EOF' > do_replace.py
import os
...
PY_EOF
python3 do_replace.py
cd unimonday-web && npm run build
